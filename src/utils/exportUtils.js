...
export function exportToExcel(selectedClasses) {
  ...
  const listData = [
    ['STT', 'Mã HP', 'Tên môn học', 'Mã lớp', 'Số TC', 'Giảng viên', 'Lịch học', 'Phòng', 'Tuần học', 'CT Đào tạo'],
  ];

  selectedClasses.forEach((cls, idx) => {
    const scheduleStr = cls.schedule.map(s =>
      `Thứ ${s.thu}, Tiết ${s.tietBD}-${s.tietBD + s.soTiet - 1}`
    ).join('; ');

    const phongStr = cls.schedule.map(s => s.phong).filter(Boolean).join('; ');
    const tuanStr = cls.schedule.map(s => s.tuanHoc).filter(Boolean).join('; ');
  ...
  ws1['!cols'] = [
    { wch: 5 }, { wch: 12 }, { wch: 35 }, { wch: 15 }, { wch: 8 },
